...
def training(rank, world_size, start_epoch, max_epoch, model, dataset, collate_fn, port):
    ...
    loader = DataLoader(dataset[start: end], batch_size=16, shuffle=True, num_workers=1, collate_fn=collate_fn, persistent_workers=True)
    ...
    decay =  torch.optim.lr_scheduler.PolynomialLR(optim, total_iters=int(iter_step*2), power=2)
    scheduler = torch.optim.lr_scheduler.SequentialLR(optim, [warm, decay], [warm.total_iters if start_epoch==0 else 1])
    ...
    for epoch in range(start_epoch, start_epoch+max_epoch):
        ...
        for i,(qa_tokens, d_tokens) in enumerate(train_bar):
            ...
            ref_logp, (LM_output, loss) = model.forward(qa_tokens, Doc_tokens = d_tokens, stage = int(epoch>=0))
            # kl = F.kl_div(LM_output, ref_logp, log_target=True, reduction="batchmean")
            loss = loss.mean()
            (loss).backward()
            # loss += kl.mean() * 0.1
            if i%max(int(16/(world_size*bs)),1)==0:
...
# stop rolling ! 
        ...
        dist.barrier()
        if rank==0:
            torch.save({k:v for k,v in model.module.named_parameters() if v.requires_grad}, f"save/NQ_EncLM_{epoch}.pt")
...
def main():
    ...
    Encoder = KnowEncoder_Embedding(embedding = LM.model.get_input_embeddings(), dims = num_dims, **enc_config, dtype=dtype)
    ...
    if False:
        ...
        weight = torch.load("save/NQ_EncLM_9.pt", map_location='cpu')
        weight["model.model.model.layers.0.self_attn.adaption_gate"].data.zero_()
        LM.load_state_dict(weight, strict=False)
        
        # except:
        #     print("Loading may not finish.")
        
    start_epoch = 0
    max_epoch = 4 # update epoch, not end epoch
    print('Loading dataset...')
    
    if False:
        data_path = "data/smart_train_QAD.jsonl"
        dataset = PretrainEnc(data_path=data_path, use_doc=True, use_short=True, use_long=False, num_samples = None)
        # dataset = [*dataset]*2**6
        length = 128
        collate_fn = collate(LM_dir, bert_dir, max_length=length, form="short").collate_qa_docs_embedding
    else:
        data_path = "data/smart_train_QAD.jsonl"
        dataset = PretrainEnc(data_path=data_path, use_doc=True, use_short=False, use_long=True, num_samples = None)
        # dataset = [*dataset]*2**4
        length = 128
        collate_fn = collate(LM_dir, bert_dir, max_length=length, form = "long").collate_qa_docs_embedding
    dataset = [*dataset]
